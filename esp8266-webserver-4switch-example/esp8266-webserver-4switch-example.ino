...
IPAddress ip(xxx, xxx, xxx,xxx); // YOUR IP
IPAddress gateway(xxx,xxx,xxx,xxx);
IPAddress subnet(255,255,255,0);
const char* myhostname = "<YOURHOSTNAME>";
#endif 
//-------------------------------------------------------------------------------
...
const char* www_username = "<YOURUSERID>";
const char* www_password = "<YOURPASS>";

const char* www1_username = "<ALTUSERID>";
const char* www1_password = "<ALTPASS>";

const char* www2_username = "<ALTUSERID>";
const char* www2_password = "<ALTPASS>";
...
#if defined (SMTP)
//---------------Email Info----------------------------
char EMAIL_FROM[] = "<SENDEREMAIL>";
char EMAIL_PASSWORD[] =  "<SENDMERSMTPPASS>";
char EMAIL_TO[] = "<RECIPEMAIL)";
char SUBJECT[] = "";

char EMAIL_CONTENT[] = "Hello,\r\nThis is a message from your ESP8266.\r\n";
char EMAIL_CONTENT_SECOND[]="                ";
char EMAIL_CONTENT_FINAL[256] ="";
// We'll need your EMAIL_FROM and its EMAIL_PASSWORD base64 encoded, you can use https://www.base64encode.org/
#define EMAIL_FROM_BASE64 "<FROMBASE64>"
#define EMAIL_PASSWORD_BASE64 "<PASSBASE64>"
#define HOST     "<SMTPHOST>"   
#define PORT     25        //or 587 depending on your provider and if STARTTLS is required      
#endif 
#if defined (POSTMARK)
...
boolean initWifiClient() {
  ...
  Serial.print(" Wifi Client Begin:");
  Serial.print(" SSID:[");
  Serial.print(ssid);
  Serial.print("] WPA2PSK:[");
...
else if (server.authenticate(www2_username, www2_password))
{
  AuthedUser=www_username;
  return true;
}
...
boolean do_next()
{
    ...
    Serial.print("Sendng Helo...");
        // send "HELO ip_address" command. Server will reply with "250" and welcome message
         stat =  wifisendrecv("EHLO <SERVERNAME>","250",500); // ideally an ipaddress should go in place 
        Serial.println(stat);
        return stat;
        break;
    case 2:
        // send "AUTH LOGIN" command to the server will reply with "334 username" base 64 encoded
        stat= wifisendrecv("AUTH LOGIN","334 VXNlcm5hbWU6",500);
        Serial.println(stat);
        return stat;
        break;
    case 3:
        // send username/email base 64 encoded, the server will reply with "334 password" base 64 encoded
        stat=wifisendrecv(EMAIL_FROM_BASE64,"334 UGFzc3dvcmQ6",500); 
        Serial.println(stat);
        return stat;
        break;
    case 4:
...
//  -H "X-Postmark-Server-Token: <POSTMARK TOKEN>" \
//  -d "{From: '<FROMEMAIL>', To: '<TOEMAIL>', Subject: 'Hello from Postmark', HtmlBody: '<strong>Hello</strong> dear Postmark user.'}"
   #if defined(DEBUG)
  ...
  Serial.println("Sending...");
   #endif
  String PostData="{From: '<FROMEMAIL>', To: '<TOEMAIL>', Subject: '";
  PostData+="Alert From ";
  PostData+=myhostname;
  PostData+="', HtmlBody: '";

  PostData+="<strong>ESP8266:</strong>";
  PostData+=myhostname;
  PostData+="<br>";
  ...
  if (stat) {
    ...
    client.println("X-Postmark-Server-Token: <POSTMARK TOKEN>");
